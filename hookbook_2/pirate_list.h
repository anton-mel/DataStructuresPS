...
/*
 * Only if there is no pirate in the list with the same name as p, insert pirate p into the list at index idx by copying the pointer, shifting the latter part of the list one “slot” to the value.
 ...
 */
void list_sort(pirate_list *pirates);   // added -n sort type
void list_sort_v(pirate_list *pirates); // added -v sort type
void list_sort_t(pirate_list *pirates); // added -t sort type
